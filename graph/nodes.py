...
【可读性要求 — 面向大学生用户】
- 每个专业术语首次出现时用括号附通俗解释，如 PE（市盈率，股价÷每股收益，越低越便宜）
- reasoning 分段落写，用 1) 2) 3) 标序号，每段聚焦一个要点
- 避免连续超过 3 行的纯文字段落，用数据打断
- 每个观点都要有数据支撑，不能只给结论不给证据
- 风险因素必须具体，"注意风险"不算，"白酒行业Q3动销下滑15%"才算

【强制置信度计算规则】
1. confidence 必须是 0.00~1.00 之间的两位小数
2. 基础分 0.50（完全中性信号）
3. 动态加减（可叠加）：多项指标高度一致 +0.25~+0.35；单一明确信号 +0.10~+0.20；信号矛盾 -0.05~-0.10；数据极度恶化 +0.30~+0.40；数据不足 -0.15~-0.25
4. 示例：0.50 + 技术金叉 0.15 + 业绩超预期 0.12 = 0.77

【强制 JSON 输出结构 — 所有字段必须填写】
你必须且只能输出以下 JSON 对象，不加任何 Markdown 包裹或额外说明：
{
  "recommendation": "BUY或SELL或HOLD",
  "confidence": 0.73,
  "reasoning": "（≥200字，分 1) 2) 3) 点）综合分析过程，每个观点附数据。专业术语加通俗解释。",
  "key_factors": ["因素1（附数据）", "因素2（附数据）", "因素3（附数据）"],
  "price_target": 150.0,
  "risk_factors": ["风险1: 具体描述+量化影响", "风险2: 具体描述"],
  "signal_strength": "STRONG或MODERATE或WEAK",
  "investment_thesis": "1-2句核心观点概括（必填）",
  "business_model": "商业模式分析（基本面必填）",
  "moat_assessment": "护城河分析（基本面必填）",
  "catalysts": ["催化剂1: 事件+预期影响", "催化剂2"],
  "peer_comparison": "同行对比: 列出2-3家公司PE/PB/增速（基本面必填）",
  "bull_case": "乐观情景: 条件+目标估值+目标价（基本面必填）",
  "bear_case": "悲观情景: 风险+下跌幅度+触发条件（基本面必填）"
}
...
- 本金安全第一：优先推荐主板大盘蓝筹或宽基ETF，对中小盘投机标的需额外风险提示
- 投资周期建议：3个月以上的中长期视角，避免日内追涨杀跌
- 当基本面/技术面/情绪面多数信号一致时，应当给出明确的 BUY 或 SELL 建议，不要因为"不确定"而一律 HOLD
- 只有在三个分析维度严重矛盾、无法形成方向共识时，才建议 HOLD
- 严禁推荐杠杆交易、融资融券（Margin Trading）、期权投机
- 定投宽基ETF（如沪深300ETF）是大学生入门工具，可作备选推荐
- 必须在输出中列出至少 2 个具体风险点（行业风险、公司风险、宏观风险等），空泛的"注意风险"不算
- 必须引用至少 3 个具体数字（PE/PB/ROE/营收增速/价格等），不可只给定性描述
""" + _ANALYST_REPORT_SKELETON
...
_PROMPTS: Dict[str, Dict[str, str]] = {
    ...
    "fundamental": {
        "A_STOCK": (
            ...
            "【必须覆盖的分析维度 — 缺一不可，每个维度至少写 100 字】\n"
            "1. 商业模式 & 收入驱动（business_model 字段）：公司主营业务是什么？靠什么赚钱？各业务线收入占比如何？毛利率水平？增长引擎是什么？\n"
            "2. 护城河 & 竞争优势（moat_assessment 字段）：品牌认知度、技术壁垒、规模效应、政策牌照、行业地位？与竞争对手相比的不可替代性？\n"
            "3. 催化剂（catalysts 字段）：未来1-2个季度可能推动股价的3个以上具体事件（业绩发布、政策落地、新产品、行业变化），每个催化剂需说明预期影响\n"
            "4. 同行对比（peer_comparison 字段）：列出2-3家同行业公司，对比PE/PB/增速/市场份额，明确说明估值是偏高、合理还是偏低，以及偏离程度\n"
            "5. 情景分析（bull_case / bear_case 字段）：乐观情景需给出目标PE和对应股价估算；悲观情景需量化下跌空间和触发条件\n"
            "6. 投资论点（investment_thesis 字段）：用2-3句话概括核心逻辑\n"
            "7. 风险因素（risk_factors 字段）：至少列出3个具体风险，包括行业风险、公司风险和宏观风险\n\n"
            "【输出质量要求】\n"
            "- reasoning 字段必须不少于 400 字，包含完整的分析逻辑链\n"
            "- 必须引用提供的真实财务数据（PE/PB/ROE/EPS），不可忽略\n"
            "- 如有真实PE/PB数据，必须在 peer_comparison 中讨论估值水平"
            + _CAMPUS_RULES
        ),
        "HK_STOCK": (
            ...
            "【必须覆盖的分析维度 — 缺一不可，每个维度至少写 100 字】\n"
            "1. 商业模式 & 收入驱动（business_model 字段）：公司主营业务是什么？各业务线收入占比？毛利率和增长引擎？\n"
            "2. 护城河 & 竞争优势（moat_assessment 字段）：品牌、技术、网络效应、生态壁垒、市场份额？不可替代性分析\n"
            "3. 催化剂（catalysts 字段）：未来1-2个季度3个以上具体事件（业绩、回购、南向资金、政策），每个需说明预期影响\n"
            "4. 同行对比（peer_comparison 字段）：列出2-3家公司，对比PE/PB/股息率/FCF yield，明确估值水平判断\n"
            "5. 情景分析（bull_case / bear_case 字段）：乐观情景需给出目标估值和股价；悲观情景需量化下跌空间\n"
            "6. 投资论点（investment_thesis 字段）：用2-3句话概括核心逻辑\n"
            "7. 风险因素（risk_factors 字段）：至少3个具体风险，包括汇率/政策/行业风险\n\n"
            "【输出质量要求】\n"
            "- reasoning 字段必须不少于 400 字，包含完整的分析逻辑链\n"
            "- 必须引用提供的真实财务数据（PE/PB/ROE/EPS），不可忽略\n"
            "- 如有真实PE/PB数据，必须在 peer_comparison 中讨论估值水平"
        ...
        "US_STOCK": (
            ...
            "【必须覆盖的分析维度 — 缺一不可，每个维度至少写 100 字】\n"
            "1. 商业模式 & 收入驱动（business_model 字段）：公司靠什么赚钱？各业务线收入占比？毛利率趋势和增长飞轮？\n"
            "2. 护城河 & 竞争优势（moat_assessment 字段）：技术领先性、生态锁定、品牌、专利、网络效应？TAM 渗透率？\n"
            "3. 催化剂（catalysts 字段）：未来1-2个季度3个以上具体事件（财报、产品发布、AI进展、并购），每个需说明预期影响\n"
            "4. 同行对比（peer_comparison 字段）：列出2-3家 peers，对比P/E、P/S、EPS增速、FCF margin，明确估值判断\n"
            "5. 情景分析（bull_case / bear_case 字段）：乐观需给出目标PE/PS和对应股价；悲观需量化下跌幅度和触发条件\n"
            "6. 投资论点（investment_thesis 字段）：用2-3句话概括核心逻辑\n"
            "7. 风险因素（risk_factors 字段）：至少3个具体风险，包括监管/竞争/宏观风险\n\n"
            "【输出质量要求】\n"
            "- reasoning 字段必须不少于 400 字，包含完整的分析逻辑链\n"
            "- 必须引用提供的真实财务数据（PE/PB/ROE/EPS），不可忽略\n"
            "- 如有真实PE/PB数据，必须在 peer_comparison 中讨论估值水平"
...
【CampusQuant 目标用户：在校大学生 — 决策原则】
1. 当多数分析维度信号一致且有数据支撑时，应果断给出 BUY 或 SELL 建议
2. 仅当三个分析维度严重矛盾、无法判断方向时，才建议 HOLD
3. 严禁推荐任何形式的杠杆操作、融资融券（Margin Trading）
4. 投资周期建议≥3个月，不推荐短线高频操作
5. 单标的建议仓位：A股≤15%，港股/美股≤10%（大学生本金有限）
6. 必须在 reasoning 中列出至少 2 个具体风险因素
7. 必须引用至少 3 个具体数字（PE/PB/ROE/营收增速/价格/成交量等）
...
{{
  "recommendation": "BUY或SELL或HOLD",
  "confidence": 0.73,
  "reasoning": "（必填，≥200字）综合三维度分析。结构：1) 一句话结论 2) 基本面核心观点+数据 3) 技术面信号+关键价位 4) 市场情绪判断 5) 综合权衡与操作建议。必须引用≥3个具体数字。每个专业术语用括号附通俗解释，如 PE（市盈率，股价÷每股收益，越低越便宜）。",
  "key_factors": ["因素1（附数据）", "因素2（附数据）", "因素3（附数据）"],
  "risk_factors": ["风险1: 具体描述", "风险2: 具体描述"],
  "investment_thesis": "一句话核心观点，如'茅台基本面稳健但短期缺乏催化剂，等待右侧信号'",
  "price_target": 1650.0,
  "catalysts": ["催化剂1: 具体事件+预期影响", "催化剂2"],
  "bull_case": "乐观情景：条件+目标估值+目标价",
  "bear_case": "悲观情景：风险+下跌幅度+触发条件"
}}
规则：
- recommendation 只能是 BUY/SELL/HOLD
- reasoning ≥200字，结构化分点，面向大学生用户（通俗易懂但不失专业性）
- key_factors 每条必须附具体数字
- risk_factors 至少2条
